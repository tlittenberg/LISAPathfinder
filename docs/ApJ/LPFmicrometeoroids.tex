...
\red{Describe the MCMC tool and what tests we did on it. Maybe grab some of Tyson's notes}
...
In this paper we restrict our analysis to segments of data where no signals were deliberately injected into the LPF system. We identified a total of XXX hours of data in three distinct configurations: the nominal LPF configuration in which the European-provided DFACS control system and cold gas micropropulsion system were operating (XXX hours), the DRS configuration in which the NASA-provided DCS control system and colloidal micropropulsion system were operating (XXX hours), and a hybrid configuration in which the DFACS was controlling the S/C using the colloidals (XXX hours). Figure \ref{fig:timeline} shows a timeline of these segments along with the detected impacts plotted with their total transferred momentum along the vertical axis. The total number of detected impacts is XXX: XXX in the nominal configuration, XXX in the DRS configuration, and XXX in the hybrid configuration. This corresponds to a rough event rate of XXX, which is broadly consistent with the estimate made in \cite{Thorpe:2015cxa}. 
...
\begin{figure}[ht!]
\fig{figures/momentumCDF.png}{0.5\textwidth}{Cumulative momentum distribution of observed impacts. \label{fig:CDF_P}}
\end{figure}
