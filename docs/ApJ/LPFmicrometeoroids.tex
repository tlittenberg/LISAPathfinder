...
%Data model
Assuming the data $\data$ is a signal $\model$ plus additive noise $\n$, and $\n$ is zero-mean Gaussian distributed, the likelihood for data $\data$ and parameters $\params$
\begin{equation}
p(\data|\params) = \frac{1}{\det \invCij} e^{-\frac{1}{2}\sum_{ij} \residual_i \invCij \residual_j }
\end{equation}
where the residual $\residual = \data - \model(\params)$ and $\model(\params)$ is the modeled LISA Pathfinder response to an impact with parameters $\params$ and the one-sided noise correlation matrix $\Cij \equiv \langle \n_i \n_j\rangle$. The indices $i$ and $j$ sum over different data channels, i.e. the 6 degrees of freedom $i:=(x,y,z,\theta,\eta,\phi)$ where $\theta$ is a rotation about $x$, $\eta$ is a rotation about $y$, and $\phi$ is a rotation about $z$.
\begin{figure}[htbp]
\begin{center}
...
The diagonal elements of the noise correlation matrix $\Cii$...
\red{Talk about BayesLine as adapted for LPF}.

%Response model -- much of this is probably redundant
\subsubsection{Impact Response}
To characterize micrometeorite strikes in the data we construct a model which maps parameters $\params$ of the impact to the signal measured $\model$ at the test mass.
The impact is modeled by six parameters: the impact time $t_0$, the magnitude of the transferred momentum $P$, the incident impact direction $\hat {\bm k}$ parameterized by a polar and azimuthal angle $\delta$ and $\alpha$, and the cartesian impact location $\rimp$ constrained to be on the surface of spacecraft (reducing the three dimensional vector to two degrees of freedom).

The search is performed on a data product known as ``$g$'', which is the equivalent free-body acceleration that the spacecraft would have had in the absence of forces applied by the control system.
To give a specific example, if the spacecraft is operating in a drag-free mode and experience an impact, the acceleration of the spacecraft caused by that impact will be compensated by applying a force to the spacecraft using the thrusters.
The $g$ product will include that commanded force divided by the mass of the spacecraft as part of the equivalent free-body acceleration.

With two test masses on LISA Pathfinder, there are two sources for $g$, named $g_1$ and $g_2$.
To derive the LISA Pathfinder response in the $g$ data products to micrometeorite impacts we begin with the dynamics equations at the spacecraft center of mass $C$.

For the linear degrees of freedom ($\lin = \{x,y,z\}$) the signal at the center of mass $\hvec^C_{\lin}$ will depend only on $t_0$, $P$, $\hat {\bm k}$, and the spacecraft mass $m$:
\begin{equation}\label{eq:lin_com}
\hvec^C_{\lin}(t) = \delta(t-t_0)P m^{-1} \hat{k}.
\end{equation}
The angular degrees of freedom ($\ang=\{\theta,\eta,\phi\}$) introduce dependence on the impact location, the location of the center of mass $\rcom$, and the moment of inertia tensor $\I$ of the spacecraft for rotations about the center of mass
\begin{equation}\label{eq:ang_com}
\hvec^C_{\ang}(t) =  \delta(t-t_0)P \I^{-1} (\rimp-\rcom) \times \hat{k}.
\end{equation}
The coordinate system in which the various spacecraft locations are defined is known as the ``body frame'' with origin located at the bottom center of the spacecraft.

Because the test masses are not at the spacecraft center of mass, the spacecraft dynamics must be transformed into the test-mass housing frame with origin at $\rgrs$. Assuming small angular accelerations, the signals from micrometeorite impacts in the linear and angular degrees of freedom for the $g$ data product are
\begin{eqnarray}\label{eq:waveforms}
\hvec_{\lin}(t)   &=& \hvec^C_{\lin} + (\rgrs - \rcom)\times\hvec^C_{\ang} \nonumber \\
\hvec_{\ang}(t) &=& \hvec^C_{\ang}.
\end{eqnarray}

\begin{figure}[htbp]
\includegraphics[width=\textwidth]{figures/coordinates.eps}
\caption{{\small Schematic showing the different vector quantities that enter into the derivation of the detector response to impacts and thruster noise.}}
\label{default}
\end{figure}
...
\item Bayes factors for detection.
\item Marginalized  posteriors for parameter estimation
\item Tests with injections
\item Consistency tests using g1 and g2
\item Caveats
\begin{itemize}
\item imperfect signal model
\item imperfect noise model
\item will lower detection efficiency.
\item will cause biases.
\item shouldn't increase false alarms....
\end{itemize}
\end{itemize}
