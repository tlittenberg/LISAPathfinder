...
def getfinite(df,i):
	...
	if len(facenum['face']) == 1: 		#if there is only one value in a face, just drop it. need [] or len > 1 for hist
...
def flatten_LPF(df, N = 50, scale = 'lin', mednmom = "TEST",currun ="TEST", cmap = colormap.parula, dirpath = "", rundirpath = ""):
	...
	for i in faces:
		...
		# finds what percentage of hits are on that face
		lennums.append((len(facenum['face'])) * 100 / len(df['face']))
